...
class IconCache private constructor() {
	...
	fun renderForeground(drawable:AdaptiveIconDrawable): Bitmap {
		...
		val recommand = { side:Int -> SIZE }
		val slice = { side:Int -> -36 }
	...
	fun getIconForeground(ctx:Context, pkg:String):Bitmap? = get(pkg, {
		...
		try {
			...
			it.foreground = if (icon is AdaptiveIconDrawable) {
				// Log.d(T, "foreground $pkg $SIZE")
				renderForeground(icon)
			} else {
	...
	fun getIconBackground(ctx:Context, pkg:String):Bitmap? = get(pkg, {
		...
		try {
			val icon = ctx.getPackageManager().getApplicationIcon(pkg)
			it.background = if (icon is AdaptiveIconDrawable) {
				renderBackground(icon).huefy()
			} else {
				renderIcon(icon).huefy()
			}
		} catch (ignored:Exception) {
	...
	fun getIcon(ctx:Context, pkg:String) = get(pkg, {
		...
		if (icon is AdaptiveIconDrawable) {
			...
			if (foreHues.size == 1) { // 前景单色
				it.icon = Alphaize.process(fore, Alphaize.keyByAlpha).toIcon()
				...
				if (foreHues.size == 1) { // 有一色是黑白灰
					it.icon = Alphaize.process(fore, Alphaize.keyByPlus, Alphaize.autoLevel).toIcon()
					// it.color = Color.BLACK
				} else {
					it.icon = Alphaize.process(fore, Alphaize.keyByMinus, Alphaize.autoLevel).toIcon()
					// it.color = Color.BLACK
				}
				it.color = fore.majorColor(Alphaize.colorful) ?: back.majorColor(Alphaize.colorful) ?: Color.BLACK
			} else if (backHues.size <= 3) {
				foreHues.removeAll { it.key == HUE_GRAY }
				it.icon = Alphaize.process(fore, Alphaize.keyByPlus, Alphaize.removeMajor, Alphaize.autoLevel).toIcon()
				// it.color = Color.BLACK
				it.color = back.majorColor(Alphaize.colorful) ?: fore.majorColor(Alphaize.colorful) ?: Color.BLACK
			} else {
				val bitmap = renderIcon(icon)
				it.icon = Alphaize.process(bitmap, Alphaize.keyByMinus).toIcon()
				// it.color = Color.BLUE
			...
			if (majorHue == HUE_GRAY) { // 背景色是黑白灰
				it.icon = Alphaize.process(bitmap, Alphaize.keyByMinus, Alphaize.removeMajor, Alphaize.autoLevel).toIcon()
				// it.color = Color.YELLOW
			} else {
				it.icon = Alphaize.process(bitmap, Alphaize.keyByRemoveHue(majorHue), Alphaize.autoLevel).toIcon()
				// it.color = Color.RED
			}
	...
	fun renderIcon(drawable:Drawable): Bitmap {
		// val recommand = { width:Int -> if (width > 0) width * 72 / 108 else 72 }
		// val slice = { width:Int -> if (width > 0) width * -18 / 72 else -18 }
		val recommand = { side:Int -> SIZE }
		return render(drawable, recommand, recommand)
	}
	...
	fun getAppColor(ctx:Context, pkg:String):Int = get(pkg, {
		...
		if (icon is AdaptiveIconDrawable) {
			val fore = renderForeground(icon)
			val back = renderBackground(icon)
			it.color = fore.majorColor(Alphaize.colorful) ?: back.majorColor(Alphaize.colorful)
		} else {
			val bitmap = renderIcon(icon)
			it.color = bitmap.majorColor(Alphaize.colorful)
		}
	}).color ?: Color.BLACK
	...
	companion object {
		...
		@JvmStatic fun render(drawable:Drawable,
			...
			// Log.d("SmallIcon", "render $width, $height");
...
fun Bitmap.toIcon(): Icon {
	val icon = Icon.createWithBitmap(this);
	// Log.d("SmallIcon", "$icon bitmap: ${this.width}, ${this.height}");
	return icon;
}
